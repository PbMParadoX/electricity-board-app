...
const EditUser = () => {
  ...
  const fetchUser = async () => {
    try {
      ...
      // Correctly parse Date_of_Application
      const [day, month, year] = fetchedUser.Date_of_Application.split('/');
      // Assuming year is in 'YY' format, prepend '20' to make it 'YYYY'
      const formattedDate = new Date(`20${year}-${month}-${day}`);
      if (isNaN(formattedDate)) {
        console.error('Invalid Date:', fetchedUser.Date_of_Application);
        // Handle invalid date, set to current date or show an error
        fetchedUser.Date_of_Application = new Date();
      } else {
        fetchedUser.Date_of_Application = formattedDate;
      }
